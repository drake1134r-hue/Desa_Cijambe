...
import { Loader2, AlertCircle, ArrowLeft, FileText, ChevronDown, Save, ImageIcon, Upload } from "lucide-react";
...
export default function EditStrukturPage() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        const res = await fetch(`/api/struktur-organisasi/${id}`, {
          cache: "no-store",
          credentials: "include",
        });
                  ...
                  <div key={field.name} className={isWide ? "sm:col-span-2" : ""}>
                    ...
                    {field.type === "textarea" ? (
                      ...
                      <div className="mt-1.5 flex flex-col gap-3 sm:flex-row sm:items-center">
                        {previews[field.name] || values.photo ? (
                          <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                            <img
                              src={previews[field.name] || values.photo}
                              alt="Foto saat ini"
                              className="h-20 w-20 flex-shrink-0 rounded-xl border border-slate-200 object-cover"
                            />
                            <div className="flex-1 text-sm text-slate-600">
                              {previews[field.name] ? (
                                <p>Pratinjau gambar baru. Simpan untuk memperbarui foto.</p>
                              ) : (
                                <p>Foto saat ini ditampilkan. Upload foto baru untuk mengganti.</p>
                              )}
                            </div>
                            <button
                              type="button"
                              onClick={() => clearFile(field.name)}
                              className="rounded-full bg-white px-3 py-2 text-xs font-semibold text-slate-700 ring-1 ring-slate-200 transition hover:bg-slate-100"
                            >
                              Hapus
                            </button>
                          </div>
                        ...
                        <label className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50/50 px-3.5 py-4 text-sm text-slate-500 transition hover:border-emerald-400 hover:bg-emerald-50/40 hover:text-emerald-700">

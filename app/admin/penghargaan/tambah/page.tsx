...
import { ArrowLeft, AlertCircle, Loader2, Save, FileText, ImageIcon, Upload } from "lucide-react";
...
export default function TambahPenghargaanPage() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      const res = await fetch(config.apiPath, { method: "POST", body: form });
    ...
    <main className="min-h-screen bg-slate-50 pt-20">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-emerald-600 via-emerald-600 to-teal-700 px-6 py-14 text-white sm:px-12">
        ...
        <div className="relative mx-auto max-w-4xl">
          ...
          <p className="mt-2 text-sm text-emerald-100/80">Lengkapi formulir di bawah untuk menambahkan data penghargaan.</p>
        </div>
      </section>
                    ...
                    {field.type === "textarea" ? (
                      ...
                      <div className="mt-1.5 flex flex-col gap-3 sm:flex-row sm:items-center">
                        ...
                        <label className="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50/50 px-3.5 py-4 text-sm text-slate-500 transition hover:border-emerald-400 hover:bg-emerald-50/40 hover:text-emerald-700">
                          ...
                          <span>{fileName[field.name] ?? "Pilih file untuk diunggah"}</span>
                          <input
                            type="file"
                            name={field.name}
                            required={field.required}
                            ...
                            onChange={(e) =>
                              setFileName((current) => ({
                                ...current,
                                [field.name]: e.target.files?.[0]?.name ?? "Pilih file untuk diunggah",
                              }))
                            }
                          />
                        </label>

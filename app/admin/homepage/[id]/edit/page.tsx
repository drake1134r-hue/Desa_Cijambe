...
export default function EditHomepagePage() {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        const res = await fetch(`/api/homepage-contents/${id}`, {
          cache: "no-store",
          credentials: "include",
        });
          ...
          <form onSubmit={handleSubmit} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid gap-5 p-6 sm:grid-cols-2 sm:p-8">
              {config.fields.map((field) => {
                ...
                const isKeyField = field.name === "key";
                return (
                    ...
                    {isKeyField ? (
                      <div className="mt-1.5">
                        <input
                          ...
                          value={value}
                          disabled
                          className="w-full rounded-xl border border-slate-200 bg-slate-100 px-3.5 py-2.5 text-sm text-slate-500 outline-none"
                        />
                        <input type="hidden" name={field.name} value={value} />
                        <p className="mt-2 text-xs text-slate-500">Key tidak dapat diubah saat edit. Jika perlu ubah section, buat konten baru dengan key berbeda.</p>
                      </div>
                    ) : field.type === "textarea" ? (
                      <textarea
                        name={field.name}

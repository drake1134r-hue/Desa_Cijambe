...
export default function AdminInfografisPage() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const res = await fetch(adminResourceConfigs.infografis.apiPath, {
          cache: "no-store",
          credentials: "include",
        });
  ...
  const handleDelete = async (id: number) => {
    ...
    setDeletingId(id);
    try {
      const res = await fetch(`${adminResourceConfigs.infografis.apiPath}/${id}`, {
        method: "DELETE",
        cache: "no-store",
        credentials: "include",
      });
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
                Kelola {adminResourceConfigs.infografis.title}
              </h1>
        ...
        <div className="mx-auto max-w-6xl">
          ...
          {items.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="border-b border-slate-200 bg-slate-50">
                    <tr>
                      {adminResourceConfigs.infografis.listFields.map((f) => (
                        <th
                          key={f.key}
              ...
              {items.length > 0 && <Pagination page={currentPageSafe} pageCount={pageCount} onPageChange={setCurrentPage} />}
            </div>
          )}
        </div>
      </section>
